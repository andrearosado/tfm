\lstset{caption=Función para pintar gráficos vectoriales escalables (SVG).,label= SVG}
\begin{SQL}
-- SELECT patches_toSVG(id::integer, geom, category, svg_color, p_area(geom)) FROM sample_patches_25830;
-- SELECT patches_toSVG(id::integer, geom, category, svg_color, p_area(geom)) FROM sample_patches_4326;

CREATE OR REPLACE FUNCTION patches_toSVG(gid integer, geom geometry, category text, svg_color text, metric metric)
RETURNS TEXT AS 
$func$ 
